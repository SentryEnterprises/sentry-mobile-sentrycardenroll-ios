...
void wrap(uint8_t* apdu_in, uint32_t in_len, uint8_t* apdu_out, uint32_t*out_len, uint8_t *key_enc, uint8_t* key_cmac)
{
    ...
    if (lc > 0)
    {
        ...
        aes_buf[lc] = 0x80;
        AES_128(key_enc, encryption_counter, iv);
    ...
    if (lc > 0) {     memcpy(cmac_buf + p, aes_buf, lcenc); p += lcenc; }

    AES_CMAC(key_cmac, cmac_buf, p, chaining_value);
        ...
        memcpy(apdu_out + pw, chaining_value, 8);  pw += 8;
